...
def roll_dice(num_rolls, dice=six_sided):
    ...
    total_points, k, sow_sad = 0, 1, False
    while k <= num_rolls:
        score = dice()
        if score == 1:
            sow_sad = True
...
def hog_pile(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    player_digit, opponent_digit = player_score % 10, opponent_score % 10
    ones_digit = 0
    if player_digit == opponent_digit:
        ones_digit = player_digit
    return ones_digit
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        # Player0's turn
        # Update Player0's score
        dice_number0 = strategy0(score0, score1)
        score0_got_from_turning = take_turn(dice_number0, score0, score1, dice, goal)
        score0 = score0 + score0_got_from_turning
        if score0 % 10 == score1 % 10:
            score0_got_from_hog_pile = hog_pile(score0, score1)
            score0 = score0 + score0_got_from_hog_pile
        ...
        # If score0 has already greater than goal, stop playing
        if score0 >= goal:
            return score0, score1
        
        # Player1's turn
        dice_number1 = strategy1(score1, score0)
        score1_got_from_turning = take_turn(dice_number1, score1, score0, dice, goal)
        score1 = score1 + score1_got_from_turning
        if score1 % 10 == score0 % 10:
            score1_got_from_hog_pile = hog_pile(score1, score0)
            score1 = score1 + score1_got_from_hog_pile
        ...
        # If score1 has already greater than goal, stop playing
        if score1 >= goal:
            return score0, score1
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        last_leader = None
    gap = abs(score0 - score1)
    
    if last_leader == previous_leader or last_leader == None:
        return last_leader, None
    else:
        message = "Player " + str(last_leader) + " takes the lead by " + str(gap)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    k, max_score, max_dice = 1, 0, 1
    while k <= 10:
        averaged_dice = make_averaged(roll_dice, total_samples)
        averaged_score = averaged_dice(k, dice)
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    score_from_hefty_hogs = hefty_hogs(score, opponent_score)
    if score_from_hefty_hogs >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_from_hefty_hogs = hefty_hogs(score, opponent_score)
    score_update = score + score_from_hefty_hogs
    if score_update % 10 == opponent_score % 10 and score_from_hefty_hogs != 0:
        return 0
