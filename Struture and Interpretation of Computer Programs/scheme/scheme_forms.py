...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_expression = Pair(formals, body) # e.g. (x y) and (+ x y) should be ((x y) (+ x y))
        lambda_procedure = do_lambda_form(lambda_expression, env)
        env.define(symbol, lambda_procedure) # Bind LambdaProcedure instance to symbol in current environment
        return symbol
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    body = expressions.rest # A Scheme list of many expressions(or just one expression)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            # If sub-expressions is nil, then return value of predicate
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_form(binding, 2, 2) # Check a binding which should only be a name and a value e.g. (x 2)
        names = Pair(binding.first, names)
        values = Pair(scheme_eval(binding.rest.first, env), values)
        bindings = bindings.rest

    validate_formals(names) # Check if it's a valid formals Scheme list
    # END PROBLEM 14
